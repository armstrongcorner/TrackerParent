...
@MainActor
protocol RegisterWithEmailUseCaseProtocol {
    func execute(email: String, password: String, flowToken: String) async throws -> AuthResponse?
}

final class RegisterWithEmailUseCase: RegisterWithEmailUseCaseProtocol, Loggable {
    ...
    func execute(email: String, password: String, flowToken: String) async throws -> AuthResponse? {
        let authDataResult = try await MTAuthHelper.shared.createAccount(withEmail: email, password: password)
